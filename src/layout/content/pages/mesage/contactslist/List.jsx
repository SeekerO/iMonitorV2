...
const List = ({
  ...
  return (
    <>
      {!openLayout && (
        <div
          className={`h-full MainColor min-w-full rounded-md md:min-w-[250px] md:rounded-l-md`}
        >
          ...
          <div className=" h-[80%] w-full mt-3 overflow-auto flex-col flex">
            {/* Cell Start */}

            {searchTerm !== "" ? (
              filteredData.map((meta_data, index) => (
                <div
                  onClick={() => handleOpenChat(meta_data)}
                  className="flex px-2 py-1 mb-2 gap-1 hover:bg-opacity-20 bg-slate-100 bg-opacity-0 cursor-pointer text-white"
                >
